...
class ISAAlgorithmHRL(ISAAlgorithmBase):
    ...
    DQN_META_MODEL_FILENAME = "meta-agent_%d-task_%d.pt"          # filename patther for deep models of q-functions over options
    ...
    def __init__(self, tasks, num_tasks, export_folder_names, params, target_automata, binary_folder_name):
        ...
        self.target_meta_q_functions = [[[{} for _ in range(self.num_tasks)] for _ in range(self.num_agents)] for _ in range(self.num_domains)]
        self.target_meta_counters = [[[{} for _ in range(self.num_tasks)] for _ in range(self.num_agents)] for _ in range(self.num_domains)]
        self.meta_optimizers = [[[{} for _ in range(self.num_tasks)] for _ in range(self.num_agents)] for _ in range(self.num_domains)]
        self.meta_experience_replay_buffers = None

        # q-functions for satisfying a particular condition
        self.policy_bank = [[{} for _ in range(self.num_tasks)] for _ in range(self.num_agents)]
        self.policy_bank_update_counter = [[{} for _ in range(self.num_tasks)] for _ in range(self.num_agents)]
        self.target_policy_bank = [[{} for _ in range(self.num_tasks)] for _ in range(self.num_agents)]
        self.target_policy_bank_counter = [[{} for _ in range(self.num_tasks)] for _ in range(self.num_agents)]
        self.policy_bank_optimizers = [[{} for _ in range(self.num_tasks)] for _ in range(self.num_agents)]
    ...
    def _build_option_tracking_structs(self):
        for domain_id in range(self.num_domains):
            # self.has_terminated[domain_id].clear()
            # self.selected_option[domain_id].clear()
            # self.last_state[domain_id].clear()
            # self.num_option_steps[domain_id].clear()
            # self.option_reward[domain_id].clear()


            for agent_id in range(self.num_agents):
                self.has_terminated[domain_id][agent_id] = {}
                self.selected_option[domain_id][agent_id] = {}
                self.last_state[domain_id][agent_id] = {}
                self.num_option_steps[domain_id][agent_id] = {}
                self.option_reward[domain_id][agent_id] = {}

                current_tasks = self.tasks[domain_id]
                for task_id in range(len(current_tasks)):
                    self.has_terminated[domain_id][agent_id][task_id] = True
                    self.selected_option[domain_id][agent_id][task_id] = None
                    self.last_state[domain_id][agent_id][task_id] = None
                    self.num_option_steps[domain_id][agent_id][task_id] = None
                    self.option_reward[domain_id][agent_id][task_id] = None
            ...
            for task_id in range(self.num_tasks):
                ...
                if task_id not in self.policy_bank:
                    ...
                    if not self.is_tabular_case:
                        self.target_policy_bank[agent_id][task_id] = {}
                        self.policy_bank_optimizers[agent_id][task_id] = {}
                        self.target_policy_bank_counter[agent_id][task_id] = {}
                ...
                for condition in automaton.get_all_conditions():
                    if condition not in self.policy_bank[agent_id][task_id] or self.always_reuse_qfunction:
                        ...
                        if not self.is_tabular_case:
                            ...
                            self.target_policy_bank[agent_id][task_id][condition].to(self.device)
                            self.target_policy_bank[agent_id][task_id][condition].load_state_dict(self.policy_bank[agent_id][task_id][condition].state_dict())
                            self.policy_bank_optimizers[agent_id][task_id][condition] = optim.Adam(self.policy_bank[agent_id][task_id][condition].parameters(), lr=self.learning_rate)
                            self.target_policy_bank_counter[agent_id][task_id][condition] = 0
    ...
    def _initialize_function_for_condition(self, task_id, task, condition):
        for agent_id in range(self.num_agents):
            if self.is_tabular_case:
                self.policy_bank[agent_id][task_id][condition] = np.zeros((task.observation_space[agent_id].n, task.action_space.n),
                                                                dtype=np.float32)
            else:
                self.policy_bank[agent_id][task_id][condition] = DQN(task.observation_space[agent_id].n, task.action_space.n, self.num_layers,
                                                        self.num_neurons_per_layer)
                self.policy_bank[agent_id][task_id][condition].to(self.device)
            self.policy_bank_update_counter[agent_id][task_id][condition] = 0

    def _initialize_function_for_condition_for_specific_agent(self, agent_id, task_id, task, condition):
        # for agent_id in range(self.num_agents):
        if self.is_tabular_case:
            self.policy_bank[agent_id][task_id][condition] = np.zeros((task.observation_space[agent_id].n, task.action_space.n),
                                                            dtype=np.float32)
    ...
    def _build_function_from_existing_condition(self, task_id, task, condition):
        ...
        for agent_id in range(self.num_agents):
            for existing_condition in self.policy_bank[agent_id][task_id]:
                ...
                if num_matchings > max_num_matchings[agent_id]:
                    max_num_matchings[agent_id] = num_matchings
                    max_conditions[agent_id] = [existing_condition]
                    max_conditions_updates[agent_id] = [self.policy_bank_update_counter[agent_id][task_id][existing_condition]]
                elif num_matchings == max_num_matchings[agent_id]:
                    max_conditions[agent_id].append(existing_condition)
                    max_conditions_updates[agent_id].append(self.policy_bank_update_counter[agent_id][task_id][existing_condition])
        ...
        for agent_id in range(self.num_agents):
            if len(max_conditions[agent_id]) > 0:
                ...
                if self.is_tabular_case:
                    ...
                    self.policy_bank[agent_id][task_id][condition].load_state_dict(self.policy_bank[agent_id][task_id][max_condition].state_dict())

    def _build_function_from_existing_condition_for_specific_agent(self, agent_id, task_id, task, condition):
        max_num_matchings = 1
        ...
        for existing_condition in self.policy_bank[agent_id][task_id]:
            ...
            if num_matchings > max_num_matchings:
                ...
                max_conditions_updates = [self.policy_bank_update_counter[agent_id][task_id][existing_condition]]
            elif num_matchings == max_num_matchings:
                max_conditions.append(existing_condition)
                max_conditions_updates.append(self.policy_bank_update_counter[agent_id][task_id][existing_condition])
    ...
    def _build_tabular_meta_q_functions(self, domain_id):
        ...
        for task_id in range(len(current_tasks)):
            ...
            for agent_id in range(self.num_agents):
                q_tables[agent_id][task_id] = {}
                
                for state in current_automaton[agent_id].get_states():
                    if current_automaton[agent_id].get_num_outgoing_edges(state) > 0:
                        q_tables[agent_id][task_id][state] = np.zeros((task.observation_space[agent_id].n, current_automaton[agent_id].get_num_outgoing_edges(state)),
                                                            dtype=np.float32)
                    else:
                        q_tables[agent_id][task_id][state] = np.zeros((task.observation_space[agent_id].n, task.action_space.n),
                                                            dtype=np.float32)
                        
    def _build_tabular_meta_q_functions_for_specific_agent(self, domain_id, agent_id):
        q_tables = self.meta_q_functions[domain_id]
        # for agent_id in range(self.num_agents):
        for task_id in range(self.num_tasks):
            q_tables[agent_id][task_id].clear()
        ...
        for task_id in range(len(current_tasks)):
            ...
            for state in current_automaton.get_states():
                if current_automaton.get_num_outgoing_edges(state) > 0:
                    q_tables[agent_id][task_id][state] = np.zeros((task.observation_space[agent_id].n, current_automaton.get_num_outgoing_edges(state)),
                                                        dtype=np.float32)
                else:
                    q_tables[agent_id][task_id][state] = np.zeros((task.observation_space[agent_id].n, task.action_space.n),
                                                        dtype=np.float32)
    ...
    def _build_dqn_meta_q_functions(self, domain_id):
        ...
        for task_id in range(len(current_tasks)):
            task = current_tasks[task_id]
            for agent_id in range(self.num_agents):
                num_states, num_actions = task.observation_space[agent_id].n, task.action_space.n
                num_automaton_states = current_automaton[agent_id].get_num_states()
    
                q_functions[agent_id][task_id] = DQN(num_states + num_automaton_states,
                                        current_automaton[agent_id].get_num_unique_conditions() + num_actions,
                                        self.num_layers, self.num_neurons_per_layer)
                q_functions[agent_id][task_id].to(self.device)
    
                target_q_functions[agent_id][task_id] = DQN(num_states + num_automaton_states,
                                                current_automaton[agent_id].get_num_unique_conditions() + num_actions,
                                                self.num_layers, self.num_neurons_per_layer)
                target_q_functions[agent_id][task_id].to(self.device)
                target_q_functions[agent_id][task_id].load_state_dict(q_functions[agent_id][task_id].state_dict())
    
                target_meta_counters[agent_id][task_id] = 0
    
                optimizers[agent_id][task_id] = optim.Adam(q_functions[agent_id][task_id].parameters(), lr=self.learning_rate)

    def _build_dqn_meta_q_functions_for_specific_agent(self, domain_id, agent_id):
        q_functions = self.meta_q_functions[domain_id]
        target_q_functions = self.target_meta_q_functions[domain_id]
        target_meta_counters = self.target_meta_counters[domain_id]
        optimizers = self.meta_optimizers[domain_id]
    ...
    def _build_experience_replay_buffers(self):
        self.experience_replay_buffers = [[ExperienceBuffer(self.experience_replay_buffer_size)
                                          for _ in range(self.num_tasks)] for _ in range(self.num_agents)]
    ...
    def _build_meta_experience_replay_buffers(self):
        ...
        # for domain_id in range(self.num_domains):
        self.meta_experience_replay_buffers = [[[ExperienceBuffer(self.experience_replay_buffer_size)
                                                    for _ in range(self.num_tasks)] for _ in range(self.num_agents)] for _ in range(self.num_domains)]
    ...
    '''
    def _choose_action(self, domain_id, agent_id, task_id, current_state, automaton, current_automaton_state_id):
        ...
        if self.has_terminated[domain_id][agent_id][task_id]:
            ...
            self.last_state[domain_id][agent_id][task_id] = current_state
            self.num_option_steps[domain_id][agent_id][task_id] = 0
    ...
    def _choose_egreedy_option(self, domain_id, agent_id, task_id, current_state, automaton, automaton_state):
        ...
        if self.train_model:
            ...
            if prob <= self.exploration_rate:
                return self._get_random_option(domain_id, agent_id, task_id, automaton, automaton_state)
    ...
    def _get_greedy_option(self, domain_id, agent_id, task_id, current_state, automaton, current_automaton_state):
        """Returns the option with the highest Q-value at the given automaton state for a given state."""
        if self.is_tabular_case:
            return self._get_greedy_option_tabular(domain_id, agent_id, task_id, current_state, automaton, current_automaton_state)
        else:
            return self._get_greedy_option_deep(domain_id, agent_id, task_id, current_state, automaton, current_automaton_state)
    ...
    def _on_performed_step(self, domain_id, task_id, next_state, reward, is_terminal, observations, automaton,
        ...
        for agent_id in range(self.num_agents):
            ...
            discount = self.discount_rate ** self.num_option_steps[domain_id][agent_id][task_id]
            self.option_reward[domain_id][agent_id][task_id] += discount * reward[agent_id]
            self.num_option_steps[domain_id][agent_id][task_id] += 1
            self.has_terminated[domain_id][agent_id][task_id] = automaton[agent_id].is_terminal_state(current_automaton_state[agent_id]) \
            ...
            if self.train_model:
                if self.has_terminated[domain_id][agent_id][task_id]:  # if the option terminates, the metacontroller is updated
                    if self.is_tabular_case:
                        self._update_tabular_meta_q_functions(domain_id, agent_id, task_id, next_state[agent_id], is_terminal[agent_id], automaton[agent_id],
                                                            current_automaton_state[agent_id], next_automaton_state[agent_id])
                    else:
    ...
    def _update_tabular_meta_q_functions(self, domain_id, agent_id, task_id, next_state, is_terminal, automaton, current_automaton_state, next_automaton_state):
        """Applies SMDP Q-Learning update to the function for the given (domain, task) pair."""
        meta_q_function = self.meta_q_functions[domain_id][agent_id][task_id]
        delta = self.option_reward[domain_id][agent_id][task_id]

        if not is_terminal:
            if automaton.get_num_outgoing_edges(next_automaton_state) > 0:
                next_greedy_option = self._get_greedy_option(domain_id, agent_id, task_id, next_state, automaton, next_automaton_state)
                next_greedy_option_id = automaton.get_outgoing_condition_id(next_automaton_state, next_greedy_option)
            else:
                next_greedy_option_id = self._get_greedy_option(domain_id, agent_id, task_id, next_state, automaton, next_automaton_state)
            discount = self.discount_rate ** self.num_option_steps[domain_id][agent_id][task_id]
            delta += discount * meta_q_function[next_automaton_state][(next_state, next_greedy_option_id)]

        if automaton.get_num_outgoing_edges(current_automaton_state) > 0:
            selected_option_id = automaton.get_outgoing_condition_id(current_automaton_state, self.selected_option[domain_id][agent_id][task_id])
        else:
            selected_option_id = self.selected_option[domain_id][agent_id][task_id]

        updated_so_pair = (self.last_state[domain_id][agent_id][task_id], selected_option_id)
    ...
    def _update_deep_meta_q_functions(self, domain_id, task_id, next_state, is_terminal, automaton, current_automaton_state, next_automaton_state): # FIX for use with GPU
    ...
    def _add_to_meta_experience_replay_buffer(self, domain_id, task_id, next_state, is_terminal, automaton, current_automaton_state, next_automaton_state): # FIX depended by GPU based function
    ...
    def _update_deep_meta_q_functions_from_batch(self, domain_id, task_id, experience_batch): #FIX for use with GPU
    ...
    def _get_available_option_mask(self, task, automaton, automaton_state): # FIX GPU baed function
        all_conditions = automaton.get_all_conditions()
        if automaton.get_num_outgoing_edges(automaton_state) > 0:
    ...
    def _get_state_and_automaton_state_vector(self, state, automaton, automaton_state): # FIX GPU baed function
    ...
    def _get_all_automata_conditions(self, agent_id):
        all_conditions = set()
        for domain_id in range(self.num_domains):
            all_conditions.update(self.automata[domain_id][agent_id].get_all_conditions())
        return sorted(all_conditions)

    def _update_tabular_q_functions(self, agent_id, task_id, current_pair, next_state, is_terminal, is_goal_achieved, observations):
        # take any task, just needed for the number of actions
        task = self._get_task(0, task_id)

        if self.update_all_policy_bank:
            conditions = self.policy_bank[agent_id][task_id]
        else:
            conditions = self._get_all_automata_conditions(agent_id)
    ...
    def _update_deep_q_functions(self, agent_id, task_id, experience_batch): # FIX : update for use with GPU
    ...
    def _get_transition_rewards_terminal_batches(self, condition, observations, is_terminal, is_goal_achieved): #FIX depend on a GPU based function 
    ...
    def _update_target_deep_q_functions(self, domain_id, task_id): # FIX for GPU use
        self._update_target_deep_meta_q_functions(domain_id, task_id)
        self._update_target_deep_policy_bank(domain_id, task_id)

    def _update_target_deep_meta_q_functions(self, domain_id, task_id): # FIX for GPU use
    ...
    def _update_target_deep_policy_bank(self, domain_id, task_id): # FIX for GPU use
    ...
    def _reset_q_functionsfor_specific_agent(self, domain_id, agent_id):
        ...
        if self.is_tabular_case:
            self._build_tabular_meta_q_functions_for_specific_agent(domain_id, agent_id)
        else:
            self._build_dqn_meta_q_functions_for_specific_agent(domain_id, agent_id)

    def _reset_meta_experience_replay_for_specific_agent(self, domain_id, agent_id):
        if self.use_experience_replay:
            # for agent_id in range(self.num_agents):
            for task_id in range(self.num_tasks):
                self.meta_experience_replay_buffers[domain_id][agent_id][task_id].clear()
